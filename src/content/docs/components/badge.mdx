...
<div class="not-content">
  ...
  <div style="margin-top: 1rem; display: flex; gap: 1rem; flex-wrap: wrap; align-items: center;">
    <Badge size="small" isPill>Small</Badge>
    <Badge size="medium" isPill>Medium</Badge>
    <Badge size="large" isPill>Large</Badge>
    <Badge isCircular srLabel="1 notification">1</Badge>
    <Badge type="info" isButton isCircular animateOnHover srLabel="Go to notifications">
...
<Aside type="caution" title="Icon or number only badges">
  When using badges with only icons or numbers, always provide the `srLabel` prop to ensure screen reader users understand the badge's purpose. This is especially important for notification counts and status indicators.
</Aside>
...
| `srLabel` | `string` | `''` | Additional context for screen readers (visually hidden) |
...
- Provides screen reader context with the `srLabel` prop

<Aside type="tip" title="Screen reader labels">
  The `srLabel` prop is essential for badges that display numbers or icons without text. It provides additional context that's only announced to screen readers while maintaining the visual design. For example, a badge showing "5" could have an srLabel of "5 new messages" for better accessibility.
</Aside>

<Aside type="caution" title="Interactive badges">
  When using badges as buttons, ensure they have clear purpose and adequate touch target size for mobile users, preverabely using the `large` size. A good use case for a button badge could be to for example remove filters from a list.
</Aside>
